def e_(fayl_adi, name, slep, siyahi):
	f = open(f"./SancaklarUsersBot{fayl_adi}.py", "x")
...
async def _(misaki):
	for i in a:
		...
		sleep({slep})
Help = CmdHelp("SancaklarUsersBot{fayl_adi}")
Help.add_command("{name}", None, "Bu plugin @SancaklarUsersBot tarafından hazırlanmıştır.")
Help.add()
								""")
	return f.close()
...
async def misakimusic(misaki):
    ...
    while netice is False:
            ...
            if rast.description == IFACI:
                ...
                await misaki.client.send_file(misaki.chat_id, yukle, caption="@SancaklarUsersBot sizin için `"+rast.description+" - "+rast.title+"` müziğini seçti iyi dinlemeler. :)")
                await event.delete()
                os.remove(yukle)
                netice = True
Help = CmdHelp("SancaklarUsersBot{fayl_adi}")
Help.add_command("{name}", None, "Bu Plugin @SancaklarUsersBot Tərəfindən Hazırlanmışdır..")
Help.add()
		""".format(
siyahi=siyahi,
name=name,
