...
for dil in bot.iter_messages(pchannel, filter=InputMessagesFilterDocument):
    if ((len(dil.file.name.split(".")) >= 2) and (dil.file.name.split(".")[1] == "epicjson")):
        if path.isfile(f"./userbot/language/{dil.file.name}"):
            try:
                ...
                if path.isfile("./userbot/language/DEFAULT.epicjson"):
                    LOGS.warn("Varsayılan dil dosyası kullanılıyor...")
                    LANGUAGE_JSON = loads(open(f"./userbot/language/DEFAULT.epicjson", "r").read())
                else:
...
if LANGUAGE_JSON == None:
    if path.isfile(f"./userbot/language/{LANGUAGE}.epicjson"):
        try:
            LANGUAGE_JSON = loads(open(f"./userbot/language/{LANGUAGE}.epicjson", "r").read())
        except JSONDecodeError:
