...
import os
from userbot import (CMD_HELP, EPİC_VERSION, DEFAULT_NAME, WHITELIST, MYID, ASISTAN, bot, SEVGILI) # Yakında
...
async def bot_ver(event):
    ...
    if which("git") is not None:
        ...
        await event.edit(f"=== {EPİC_VERSION} === "
                         f"`{LANG['VERSION']}: "
                         f"{verout}"
                         "` \n"
                         f"`{LANG['REVOUT']}: "
...
async def asistanalive(ups):
    ...
    if ups.is_reply:
        ...
        if ren == MYID:
            EpicVer = str(EPİC_VERSION.replace("v","")) 
            await ups.reply(f"__{hitap} Şuan Çalışmaktayım\n BotVer: {EPİC_VERSION} !__")
        else:
            return
    else:
        return
