...
PAKET_ISMI = os.environ.get("PAKET_ISMI", "| 🌃 @EpicUserBot Paketi |")
    ...
    try:
        ...
        async def start_bot_handler(event):
            if not event.message.from_id == uid:
                await event.reply(f'`Merhaba ben` @EpicUserBot`! Ben sahibime (`@{me.username}`) yardımcı olmak için varım, yaani sana yardımcı olamam :/ Ama sen de bir Epic açabilirsin; Kanala bak` @EpicUserBot')
            else:
        ...
        async def inline_handler(event):
            ...
            if event.query.user_id == uid and query == "@EpicUserBot":
                ...
                result = builder.article(
                    "@EpicUserBot",
                    text="""@EpicUserBot'u kullanmayı deneyin!
Hesabınızı bot'a çevirebilirsiniz ve bunları kullanabilirsiniz. Unutmayın, siz başkasının botunu yönetemezsiniz! Alttaki GitHub adresinden tüm kurulum detayları anlatılmıştır.""",
                    buttons=[
                        [custom.Button.url("Kanala Katıl", "https://t.me/EpicUserBot"), custom.Button.url(
                            "Gruba Katıl", "https://t.me/HydraDestek")],
                        [custom.Button.url(
        ...
        async def sayfa(event):
            if not event.query.user_id == uid: 
                return await event.answer("❌ Hey! Benim mesajlarımı düzenlemeye kalkma! Kendine bir @EpicUserBot kur.", cache_time=0, alert=True)
            sayfa = int(event.data_match.group(1).decode("UTF-8"))
            veriler = butonlastir(sayfa, CMD_HELP)
            await event.edit(
                f"** En Gelişmiş UserBot!** [Epic](https://t.me/EpicUserBot) __Çalışıyor...__\n\n**Yüklenen Modül Sayısı:** `{len(CMD_HELP)}`\n**Sayfa:** {sayfa + 1}/{veriler[0]}",
                buttons=veriler[1],
                link_preview=False
            )
        ...
        LOGS.info(
            ...
            "Eğer bunun dışında bir sorun olduğunu düşünüyorsanız bize ulaşın t.me/HydraDestek."
        )
